...
const AddBaby = () => {
  ...
  const [errors,setErrors]= useState({})
    ...
    var today = new Date();
    let mon = (today.getMonth() + 1)<=9? ('0'+(today.getMonth() + 1)):(today.getMonth() + 1),
...
<Form.Group className="mb-2" id={d.coll2}>
 <Form.Control type="text" value={vals.name} placeholder="Enter Username" required onChange={handleInput} id={d.controlx} name='name' x="true"/>
       <Form.Label id={d.label}  class="form-control-placeholder transition">Baby Name</Form.Label>
       {errors.name && <Alert key="danger" variant="danger" id={d.v}>{errors.name}</Alert>}

 </Form.Group>      

<Form.Group id={d.coll2}>
                     <Form.Control type="date"  placeholder="Due date" required id={d.controlx} value={vals.birthDate} onChange={handleInput}  x="true" name='birthDate' min={last} max={date} />
                     <Form.Label id={d.label}  class="form-control-placeholder transition">Birth Data</Form.Label>
</Form.Group>

<Form.Group id={d.coll2}>
                     <Form.Control type="number" placeholder="Enter weight in Gm" max={20} min={10} required id={d.controlx} value={vals.weight} onChange={handleInput} name='weight' x="true" />
                     <Form.Label id={d.label}  class="form-control-placeholder transition">weight</Form.Label>
</Form.Group>

<Form.Group id={d.coll2}>
                     <Form.Control type="number" placeholder="Enter head Diameter in cm" max={20} min={10} required id={d.controlx} value={vals.headDiameter} name='headDiameter' onChange={handleInput}  x="true" />
                     <Form.Label id={d.label}  class="form-control-placeholder transition">head Diameter</Form.Label>
</Form.Group>

<Form.Group id={d.coll2}>
                     <Form.Control type="number" placeholder="Enter height in cm" max={20} min={10} required id={d.controlx} name='height' value={vals.height} onChange={handleInput}  x="true" />
                     <Form.Label id={d.label}  class="form-control-placeholder transition">Height</Form.Label>
</Form.Group>
