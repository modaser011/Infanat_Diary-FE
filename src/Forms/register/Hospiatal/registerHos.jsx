        ...
        address:'qantara3'})
          ...
          {
        e.preventDefault();
        setErrors(HosValidate(vals)) 
        let xc=HosValidate(vals)
        if(xc.name===""&&xc.password===""&&xc.confirmpassword==="")
        {
          var json = JSON.stringify(vals)
          console.log(json)
          axios.post('https://infant-diary-backend.onrender.com/api/v1/hospital/signup', json,{headers:{'Content-Type':'application/json'}})
        .then(res=>{
          if(res.status === 200) {
            nav('/');
        } else {
